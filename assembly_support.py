...
def bam_to_annotate(bam, id_csv, outdir, est_fragments):
    ...
    for read in inbam.fetch(until_eof=True):
        ...
        if read.reference_name:
            ...
            if est_fragments:
                read_info += ['-1']
        if read.has_tag('BX'): # If the BAM was generated by EMA, add the barcode to the array
...
def make_barcode_whitelist(in_fq, outdir):
    ...
    curr_bc = ''
    with open(in_fq, 'r') as fq, open(join(outdir, prefix(in_fq) + '.whitelist.txt'), 'w') as outf:
        for i, line in enumerate(fq):
            if (i % 4) == 0 and 'BX:Z:' in line:
                bc = line.strip().replace(' BX:Z:','-').split('-')[1]
                if bc != curr_bc:
                    outf.write(bc + '\n')
                    curr_bc = bc
                if (i % 1000000) == 0:
