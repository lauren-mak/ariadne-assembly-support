...
def fastq_to_table(fastq, mapping_tbl, outdir):
    """Generates tables that pair enhanced cloud numbers with the actual reference sequence. Output is [read, barcode, enhanced_num, reference name]"""
...
def load_tbl(tbl):
    with open(tbl, 'r') as tf:
        out = list(reader(tf)) # Skip the header
    return out
...
def merge_clouds(forward_lst, reverse_lst, sl):
    ...
    for i in range(len(forward_lst)):
        ...
        if barcode and barcode != curr_barcode:
            for c in cloud_ref_dict:
                species_cts = count_species(cloud_ref_dict[c], sl)
                species_info.append([barcode, c] + species_cts)
                size, purity, entropy = evaluate_cloud(species_cts)
                summary_info.append([barcode, c, size, purity, entropy])
...
def dataset_summary(cloud_counts, prefix, outdir):
    ...
    logger('Finished generating dataframe, adding frequency information')
    plt.plot(size_df)
    plt.savefig(outdir + '/' + prefix + '.dataset.png', format = 'png', dpi = 1200)
...
@click.argument('outdir') # Analysis output directory
def generate_summaries(forward_tbl,reverse_tbl,species,outdir): 
    ...
    prefix = basename(forward_tbl).split('.')[0]
    dataset_summary(cloud_counts, prefix, outdir)
    with open(outdir + '/' + prefix + '.species.csv', 'w') as of: # Barcode,Cloud_Num,Species_0,Species_1,Species_2...
        ow = writer(of)
        ow.writerow(['Barcode', 'Cloud_Num'] + species_lst)
        ow.writerows(species_info)
    with open(outdir + '/' + prefix + '.summary.csv', 'w') as of:
        ow = writer(of)
        ow.writerow(['Barcode', 'Cloud_Num', 'Size', 'Purity', 'Entropy'])
        ow.writerows(summary_info)


def load_dataframes(file_prefixes, out_dir):
    all_df_list = []
    size_filter = 2 # Exclude all read clouds that are composed of a single pair of reads 
    for fp in file_prefixes:
        df = pd.read_csv('{}/{}.summary.csv'.format(out_dir,fp), header = 0)
        filtered_df = df[df['Size'] > size_filter]
        all_df_list.append(filtered_df)
    return all_df_list


def make_main_graphs(all_df_list, distances, param_name, out_dir):
    ...
    fig.savefig('{}/{}.png'.format(out_dir, param_name), format = 'png', dpi = 1200)
    plt.clf()
    ...
    for ax in axs.flat:
        ax.label_outer()
    fig.savefig('{}/{}_scaled.png'.format(out_dir, param_name), format = 'png', dpi = 1200)


def make_secondary_graphs(param_list, distances, param_name, out_dir): 
    # Matplotlib settings needed to create complex bar plots
    mpl.rcParams['font.size'] = 6
    mpl.rcParams['figure.dpi'] = 250
    scale_y = 1e3
    ...
    fig.savefig('{}/{}.png'.format(out_dir, param2strng), format = 'png', dpi = 1200)
    df = pd.DataFrame(list(zip(distances,param_list)), columns=['Search_Distances', param2strng])
    df.to_csv('{}/{}.tbl'.format(out_dir, param2strng))
...
def evaluate_clouds(distances, prefixes, out_dir):
    """Make summary graphs of the summary information table from generate_summaries()."""
    start_time = datetime.now()
    click.echo(f'Generating summary statistics for search distances {distances} of the dataset {"_".join(out_dir.split("_")[:-1])}', err=True)

    search_distances = distances.split(',')
    file_prefixes = prefixes.split(',')
    all_df_list = load_dataframes(file_prefixes, out_dir) 
    ...
    make_main_graphs(all_df_list, search_distances, 'Purity', out_dir)
    make_main_graphs(all_df_list, search_distances, 'Entropy', out_dir)
    ...
    # make_secondary_graphs(num_read_clouds, search_distances, 'Num. Read Clouds', out_dir)
    # What is the average size of a read-cloud?
    avg_cloud_size = list(map(lambda df: np.mean(df['Size']), all_df_list))
    make_secondary_graphs(avg_cloud_size, search_distances, 'Avg. Cloud Size', out_dir)
    # # How many reads were deconvolved?
    # num_reads_decon = list(map(lambda df: df['Size'].sum(), all_df_list[1:]))
    # make_secondary_graphs(num_reads_decon, search_distances[1:], 'Num. Read Deconv.', out_dir)
    # What is the average purity of the read clouds?
    avg_cloud_purity = list(map(lambda df: np.mean(df['Purity']), all_df_list))
    make_secondary_graphs(avg_cloud_purity, search_distances, 'Avg. Cloud Purity.', out_dir)
    # What is the average purity of the read clouds?
    avg_cloud_entropy = list(map(lambda df: np.mean(df['Entropy']), all_df_list))
    make_secondary_graphs(avg_cloud_entropy, search_distances, 'Avg. Cloud Entropy.', out_dir)
